...
import { Mail, Phone, MapPin, Github, Linkedin, Menu, X, Code, Server, Database, BrainCircuit, Layers, Briefcase, Star, Lightbulb, FileText, ArrowRight, Cog, Cloud, MoveVertical, DatabaseZap, GraduationCap, Users, Award } from 'lucide-react';
...
const TimelineCard = ({ role, company, duration, description }) => (
  <div className="relative pl-8 md:pl-12 py-4 border-l-2 border-cyan-700/50 group">
    ...
    <p className="text-cyan-300 mb-1">{company} | {duration}</p>
...
export default function App() {
  ...
  const navLinks = ["About", "Skills", "Experience", "Education", "Projects", "Community", "Contact"];
  ...
  return (
    <div className="bg-gray-950 text-gray-300 leading-relaxed selection:bg-cyan-300 selection:text-black relative overflow-x-hidden">
      ...
      {/* Scanline Effect */}
      <div className="absolute inset-0 z-0 opacity-[0.03]" style={{ backgroundImage: "repeating-linear-gradient(0deg, transparent, transparent 1px, #fff 1px, #fff 2px)" }}></div>
      
      {/* All content goes into a relative container to sit above the scanlines */}
      <div className="relative z-10">
        {/* Header */}
        <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isScrolled || isOpen ? 'bg-gray-950/80 backdrop-blur-sm shadow-md border-b border-cyan-700/30' : 'bg-transparent'}`}>
          <div className="max-w-7xl mx-auto px-6 md:px-8">
            <div className="flex justify-between items-center h-20">
              <a href="#" className="text-2xl font-bold text-gray-100 tracking-wider font-display">
                <span className="text-cyan-300">A</span>H.
              </a>
              <nav className="hidden md:flex flex-wrap gap-x-6">
                {navLinks.map(link => (
                  <a key={link} href={`#${link.toLowerCase()}`} className="text-gray-300 hover:text-cyan-300 transition-colors duration-300 font-medium tracking-wide">{link}</a>
                ))}
              </nav>
              <div className="md:hidden">
                <button onClick={() => setIsOpen(!isOpen)} className="text-gray-100 z-50">
        ...
        <AnimatePresence>
          {isOpen && (
            ...
            >
              ...
              {navLinks.map(link => (
                <a key={link} href={`#${link.toLowerCase()}`} onClick={() => setIsOpen(false)} className="text-2xl text-gray-300 hover:text-cyan-300 transition-colors duration-300 tracking-wide">{link}</a>
              ))}
            </nav>
            </motion.div>
          )}
        ...
        <main>
          {/* Hero Section */}
          <section id="home" className="h-screen min-h-[700px] flex items-center justify-center bg-transparent px-6 md:px-8 relative">
...
S                  >
                      <a href={portfolioData.socials.github} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-cyan-300 transition-colors duration-300"><Github size={32} /></a>
                      <a href={portfolioData.socials.linkedin} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-cyan-300 transition-colors duration-300"><Linkedin size={32} /></a>
                  </motion.div>
              </div>
          </section>
...
Access-Control-Allow-Origin: *
          ...
          <Section id="community" title="Community">
              <div className="max-w-3xl mx-auto">
                  ...
                  <h3 className="text-2xl font-bold text-cyan-300 mt-16 mb-8 text-center">Certifications</h3>
          ...
          <Section id="contact" title="Get In Touch">
              <div className="max-w-2xl mx-auto text-center">
                  ...
                  <a href={`mailto:${portfolioData.email}`} className="inline-block bg-cyan-600/50 hover:bg-cyan-600/80 border border-cyan-500 text-cyan-200 font-bold py-3 px-8 rounded-lg text-lg transition-all duration-300 tracking-wider">
                     Say Hello
                  </a>
